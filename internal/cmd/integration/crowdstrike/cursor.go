...
// cursorCmd groups maintenance of the saved sync cursor. The cursor lets `run`
...
func cursorCmd(a *app.App) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "cursor",
		Short: "Manage the saved sync cursor for the active SafeDep profile",
		Long: `Manage the saved sync cursor for the active SafeDep profile.
...
The next run processes events after this timestamp. Use it to re-process from a
chosen point. Only the profile selected with --profile is affected.
...
The timestamp is RFC3339, for example 2026-08-25T10:00:00Z.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			drytui.Info("The next run processes events after this time")
			return nil
		},
	}
}
...
func cursorRemoveCmd(a *app.App) *cobra.Command {
	return &cobra.Command{
		Use:   "remove",
		Short: "Remove the saved sync cursor so the next run starts fresh",
		Long: `Remove the saved sync cursor for the active SafeDep profile.
