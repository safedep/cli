...
| `--disable <uuid>` | Repository UUID to remove from the allowlist. Braced and uppercase forms are accepted and normalized. Repeat for multiple repositories. A UUID not on the allowlist is a no-op. |
