...
**Architecture:** Five adapters (Claude Code, Cursor, Gemini CLI stub, OpenCode stub, Antigravity stub) each implement a single `Agent` interface with capability-query methods (`AsGlobalInjector`, `AsWorkspaceInjector`). Shared JSON helpers in `config.go` perform atomic, idempotent reads and writes on the `mcpServers.safedep` key only. `InjectAll`/`RemoveAll` in `inject.go` are the only public orchestration surface; command-layer code (Chunk C) calls these.
...
| `internal/agent/geminicli.go` | Gemini CLI stub |
| `internal/agent/opencode.go` | OpenCode stub |
| `internal/agent/antigravity.go` | Antigravity stub |
...
| `internal/agent/geminicli_test.go` | Compile + detection test for stub |
| `internal/agent/opencode_test.go` | Compile + detection test for stub |
| `internal/agent/antigravity_test.go` | Compile + detection test for stub |
